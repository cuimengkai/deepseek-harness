...
describe('SessionPersistenceSqlite physical deletion', () => {
  it('physically removes the session row and cascades its event rows', async () => {
    ...
    const fiber = await ctx.plugin(SessionPersistenceSqlite, { path })
    try {
      const header = meta('sqlite-delete', '/work')
      await ctx.sessionPersistence.create(header)
      await ctx.sessionPersistence.append(header.id, chunkLog(4))
      expect(await ctx.sessionPersistence.delete(header.id)).toBe(true)
      expect(await ctx.sessionPersistence.list()).toEqual([])
      await expect(ctx.sessionPersistence.load(header.id)).rejects.toThrow(/not found/i)

      // The declared ON DELETE CASCADE removes the event rows in the same
      // transaction as the sessions row; both must be gone at the row level.
      const db = new DatabaseSync(path)
      try {
        const sessionRows = db.prepare(testSql('count-sessions'))
          .get(header.id) as { count: number }
        expect(sessionRows.count).toBe(0)
        const eventRows = db.prepare(testSql('count-session-events'))
          .get(header.id) as { count: number }
  ...
  it('leaves the store identity and other sessions untouched by a delete', async () => {
    const path = await freshDbPath('dsh-sqlite-delete-keeper-')
    const ctx = new Context()
    await ctx.plugin(SessionStore)
    const fiber = await ctx.plugin(SessionPersistenceSqlite, { path })
    try {
      ...
      await ctx.sessionPersistence.append(keeper.id, chunkLog(4))
      ...
      await ctx.sessionPersistence.append(gone.id, chunkLog(4))
