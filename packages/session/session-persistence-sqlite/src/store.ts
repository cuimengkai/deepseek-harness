...
export class SqliteStore implements PersistenceBackend<number> {
  ...
  /**
   * Physically remove one session's `sessions` row and — via the declared
   ...
   * @returns whether a materialized session existed.
   */
  async deleteStored(id: SessionId, signal?: AbortSignal): Promise<boolean> {
    ...
    try {
      validateSchemaForMutation(this.databaseConstructor, this.db, this.databasePath)
      changes = this.db.prepare(sql('delete-session')).run(id).changes
      this.db.exec(sql('commit'))
    } catch (error: unknown) {
