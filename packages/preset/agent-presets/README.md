...
- `ctx.agentPresets.readRows(id): Promise<ComposeRow[]>` One preset's composition parsed into row structures, for a caller that renders or edits rows rather than the YAML text.
- `ctx.agentPresets.copy(from, id, name?): Promise<void>` Create a locally authored preset by copying an existing one's whole directory — the directory-copy authoring write. No composition text crosses this seam, so a copy is exactly as loadable as its source; the copied metadata keeps the source's description but never its name or roster order, and `name` (or the id fallback) is what distinguishes the rows.
- `ctx.agentPresets.compose(id, rows, meta?, { overwrite, assertResolvable }): Promise<void>` Write a composition from row structures, creating it or replacing a locally authored preset in place — the browser-facing authoring write. It enforces the preset domain's own row invariants (non-empty rows, a plugin module per row, unique ids) and refuses rows that name an uninstalled module through the required `assertResolvable` proof; the wire layer supplies the inventory-backed check, so no caller can bypass it. Replacing demands a `user` target — a preset that ships with the deployment is refused.
- `ctx.agentPresets.composeGraph(id, graph, meta?, { overwrite, assertResolvable }): Promise<void>` Write a composition from a preset composition graph, creating it or replacing a locally authored preset in place — the canvas-facing authoring write. The graph is the authoring source: each agent node's `composition` field carries the row it projects (`module` plus optional `id`/`group`/`config`/`disabled`/`inject`), the rows that mount are derived here and validated exactly as `compose()` validates them, and one authoring primitive writes both files — `agent.cordis.yml` from the derived rows and `agent.flow.json` beside it holding the graph as authored. A graph with a condition or loop node, an agent node without a `composition` module, or a cycle is refused before any write.
- `ctx.agentPresets.readGraph(id): Promise<FlowGraph>` One preset's composition as a graph. The stored `agent.flow.json` serves only while it still projects exactly the rows parsed from the composition file — a hand edit or a legacy rows-compose write wins, and the rows are re-projected as a fresh chain graph otherwise. An older preset with no graph file regenerates on open.
...
Authoring has two paths: a whole-directory copy and a row-list compose. The copy lands an existing preset's entire directory — composition, metadata, skill directories, assets — under the first `user` root; the inputs are two ids the service resolves against its own roots plus an optional display name, so no caller supplies composition text and a copy grants nothing the roster did not already carry. The compose writes `agent.cordis.yml` from row structures the caller assembles; it enforces the row invariants and refuses any row that names an uninstalled module (the caller proves resolvability, not the service), replaces only a `user`-authored preset in place, and is never mounted to validate — loader-level checks run at mount, as they do for every authored preset. Everything else — description, skills — is edited in the preset's own files. `copy()` refuses three things before anything lands:
...
### Authoring a composition graph

The canvas authors a preset as a **composition graph**, and one write commits both files: the rows it projects become `agent.cordis.yml` (the mount source, unchanged) and the graph itself lands in `agent.flow.json` beside it, holding node ids, positions, and edges as authored. The graph is the chain projection of the composition — `start` → one agent node per row → `end`, in mount order — and each agent node's optional `composition` field carries its row exactly (`{ module, id?, group?, config?, disabled?, inject? }`), so projecting back to rows is lossless and `compose()`'s row invariants apply unchanged. Graph node ids are canvas-internal; a row's plugin id is whatever `composition.id` says, and a row without one keeps it unset.

A condition or loop node is refused — branching is the flow engine's later phase, not the preset domain — as is an agent node without a `composition` module and any graph with a cycle. The store is preset-owned and versioned: `agent.flow.json` sits beside `agent.cordis.yml`, carries `formatVersion` 1 and a 256 KiB cap, and is written atomically with the composition, so a partial dual-file write never becomes the durable state.

On open the stored graph is a layout cache rather than a second truth. `readGraph()` returns it only while `graphToRows(stored)` still equals the rows parsed from `agent.cordis.yml`; otherwise it re-projects a fresh chain graph from the rows, so a hand edit or a legacy rows-compose write wins and an older preset with no graph file regenerates on open.
...
The mounted subtree therefore overrides `write()` as a no-op. Nothing auto-writes a composition; authoring one is a separate, explicit operation — `copy()` and `compose()` are the only writes, and neither runs under a mounted subtree.
...
- **A copied preset keeps the source's composition graph** — `copy()` is a whole-directory copy, so the companion `agent.flow.json`'s layout and `name` travel verbatim while `preset.yml` is rewritten; the copy's graph serves until the preset is next composed as a graph.
