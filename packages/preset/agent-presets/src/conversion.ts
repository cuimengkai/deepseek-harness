...
import type { ComposeRow } from './types.ts'
 ...
 * @throws when the graph carries a disallowed node type or a cycle.
 */
export function graphToRows(graph: FlowGraph): ComposeRow[] {
  ...
  return ordered.flatMap((id): ComposeRow[] => {
    const node = agents.get(id)
    if (node === undefined) return []
    const composition = node.composition
    if (composition === undefined) return []
    // Fields are set one by one: spreading per-field conditional objects
    // degrades the JsonValue inference under exactOptionalPropertyTypes.
    const row: ComposeRow = { name: composition.module }
    if (composition.id !== undefined) row.id = composition.id
    if (composition.config !== undefined) row.config = composition.config
    if (composition.group !== undefined) row.group = composition.group
    if (composition.disabled !== undefined) row.disabled = composition.disabled
    if (composition.inject !== undefined) row.inject = composition.inject
    return [row]
  })
}
