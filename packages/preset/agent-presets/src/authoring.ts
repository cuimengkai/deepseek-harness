 ...
 *
 * `writeComposition` is the one sanctioned exception to "no caller supplies
 * composition text": the platform preset assembler renders a validated tree
 * and commits it through this primitive, which owns id validation, occupancy
 * refusal, mode tightening, and atomic writes. The write is NOT mounted to
 * validate; loader-level checks run at mount.
