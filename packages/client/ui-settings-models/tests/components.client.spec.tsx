...
describe('ModelsSection', () => {
  ...
  it('declares input modalities and roles on a DeepSeek catalog row', async () => {
    ...
    fireEvent.click(screen.getByText(en.apply))
    await waitFor(() => { expect(mutate).toHaveBeenCalledTimes(1) })
    const ops = mutate.mock.calls[0]?.[1] as { path: string[]; value: unknown }[] | undefined
    const value = ops?.[0]?.value as Record<string, unknown>[] | undefined
  ...
  it('marks the default provider and refuses a switch the catalog cannot serve', async () => {
    ...
    face.credentials.describe.mockImplementation((refs: string[]) =>
      Promise.resolve(remoteOk(Object.fromEntries(refs.map(ref => [ref, {
  ...
  it('switches the default model through the card command and reloads the page', async () => {
    const afterDefault = wireNamespaces().map(namespace => namespace.ns === 'agent-default-model'
      ...
      : namespace)
    const mutate = vi.fn(() => Promise.resolve(remoteOk(afterDefault[3]!)))
    ...
    face.session.modelCatalog.mockResolvedValue(remoteOk({
      groups: [{
  ...
  it('duplicates a dict-keyed provider profile under a fresh route key', async () => {
    const mutate = vi.fn(() => Promise.resolve(remoteOk(wireNamespaces()[2])))
    await mountSection({ mutate })
