...
> & {
  ...
  /** Open the browser-owned session delete-confirmation dialog (id + display title). */
...
export function WorkspaceBrowser({
  ...
  // Session delete keeps the browser-owned confirmation pattern of the
  // workspace delete below: destructive, host-cascaded over the whole
  // subagent tree, so the row menu routes through a dialog instead of the
  // archive's direct commit.
  ...
  useEffect(() => {
    if (sessionDeleteCommittedId === null || sessionById[sessionDeleteCommittedId] !== undefined) return
    setSessionDeleting(false)
    setSessionDeleteCommittedId(null)
    setSessionDeleteTarget(null)
  }, [sessionDeleteCommittedId, sessionById])
  ...
  const confirmSessionDelete = () => {
    ...
    deleteSession(sessionDeleteTarget.sessionId).then(() => {
      // Keep the confirmation pending until the list projection renders
      // without the deleted id, mirroring the workspace-delete posture.
