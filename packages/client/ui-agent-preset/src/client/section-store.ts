/**
 * Agent-preset management controller: the roster as a list, a copy dialog, a
 * read-only canvas over the shipped compositions, and the flow-graph composer
 * that authors a locally authored preset's plugin chain.
 *
 * The browser edits no composition text. A new preset is a host-side copy of
 * an existing one or a composition authored on the canvas graph (only the
 * graph and names cross the wire), and everything after creation happens in
 * the preset's own files — which is why the page's other job is getting the
 * user TO those files: open the directory where the host has a desktop, show
 * its path where it does not.
 *
...
import type { ModelCatalogFailure } from '@deepseek-ai/dsh-api-remotes/client'
...
/**
 * The configured model catalog the composer's model-kind picker reads. Each
 * kind's row filters the groups by the kinds the models declare, so the picker
 * offers the models Settings actually configures rather than free text.
 */
...
/** Reads the roster and drives the copy dialog, viewer, composer, and location reveals. */
export class AgentPresetSectionController {
  ...
  constructor(
    private readonly remote: Pick<ClientRemote, 'agentPresets' | 'settings' | 'session'>,
    /**
  ...
  async view(id: string): Promise<void> {
    ...
    try {
      const result = await this.remote.agentPresets.readGraph(id)
      if (!result.ok) {
        this.set({ error: result.error.message })
        return
      }
      const { name, graph } = result.value
  ...
  async confirmCompose(): Promise<boolean> {
    ...
    try {
      ...
      const result = await this.remote.agentPresets.saveGraph(
        draft.id,
        ...
        name === '' ? undefined : name,
        undefined,
        this.store.getSnapshot().rows.some(row => row.id === draft.id),
      )
      if (!result.ok) {
        this.patchComposer({ saving: false, error: result.error.message })
        return false
      }
      this.set({ composer: null, palette: null, modelCatalog: null })
      await this.load()
