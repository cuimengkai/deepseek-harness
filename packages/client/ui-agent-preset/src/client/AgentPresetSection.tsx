/**
 * Agent-presets settings section: the roster as cards, a copy dialog, the
 ...
 * preset is authored on the canvas graph or edited in its own files, which is
 * what the location action leads to. Deleting a preset leaves running sessions
 * alone: a composition is mounted once at session creation and nothing
 * re-reads the file.
 */
  ...
  return (
    <div className={css.section}>
      ...
      {([['system', t('builtInGroup')], ['user', t('customGroup')]] as const).map(([trust, heading]) => {
        ...
        // stays on screen even while empty: heading plus the composer entry.
