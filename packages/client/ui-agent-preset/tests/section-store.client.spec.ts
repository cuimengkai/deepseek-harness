...
interface FakeOptions {
  ...
  failCompose?: string
  /** Reject `openAgentPresetDirectory` with this message. */
  failOpen?: string
  /** Reject `deletePreset` with this message. */
  failRemove?: string
  /** Reject `settings.update` with this message. */
  failSettings?: string
  /** Reject `session.modelCatalog` with this message. */
  failModels?: string
  ...
  /** Throw from `openAgentPresetDirectory`, as a dead transport does. */
  throwOpen?: boolean
  /** Throw from `session.modelCatalog`, as a dead transport does. */
  throwModels?: boolean
  /** Throw from `settings.canOpenAgentPresetDirectory`, as a dead transport does. */
  ...
  failPalette?: boolean
  /** The model catalog `session.modelCatalog` serves; empty by default. */
...
/**
 * The Remote face over an in-memory preset store: copies and compositions
 * land, so the roster the controller re-reads after a mutation is the one the
 * mutation produced.
 ...
 * @returns the fake Remote face.
 */
function fakeRemote(
  presets: Map<string, FakePreset>,
  defaultId: { id: string },
  options: FakeOptions = {},
): Pick<ClientRemote, 'agentPresets' | 'settings' | 'session'> {
  ...
  return {
    agentPresets: {
      ...
      readGraph: (agentPreset: string) => {
        record('readGraph', { agentPreset })
        ...
        return remoteOk({
          agentPreset,
          trust: preset.trust,
          graph: preset.graph ?? emptyChainGraph(agentPreset, ''),
          ...preset.name === undefined ? {} : { name: preset.name },
        })
      },
      ...
      copy: (...args: [from: string, id: string, name?: string]) => {
        ...
        const [from, agentPreset, name] = args
        record('copy', { from, agentPreset, ...name === undefined ? {} : { name } })
      ...
      saveGraph: async (
        ...
        record('saveGraph', { agentPreset, graph, name, overwrite })
        await options.holdCompose
        if (options.throwCompose === true) throw new Error('socket closed')
        if (options.failCompose !== undefined) return await remoteFail(options.failCompose)
        const existing = presets.get(agentPreset)
        if (overwrite !== true && existing !== undefined) {
          return await remoteFail(`preset ${agentPreset} already exists`)
        }
        if (overwrite === true && existing?.trust === 'system') {
          return await remoteFail(`preset ${agentPreset} is read-only`)
        }
        presets.set(agentPreset, {
          trust: 'user',
          content: graphRows(graph).map(row => `- id: ${row.id}\n`).join(''),
          graph,
          ...name === undefined ? {} : { name },
        })
        return await remoteOk({ agentPreset })
      },
    ...
    session: {
      modelCatalog: () => {
        ...
        if (options.failModels !== undefined) return remoteFail(options.failModels)
        return remoteOk({ groups: options.modelGroups ?? [], failures: [] })
      },
    },
  } as unknown as Pick<ClientRemote, 'agentPresets' | 'settings' | 'session'>
}
...
function harness(options: FakeOptions = {}) {
  ...
  const controller = new AgentPresetSectionController(
    fakeRemote(presets, defaultId, { ...options, calls: options.calls ?? calls }),
    () => { rosterChanges += 1 },
    {
      list: async () => {
...
describe('the copy dialog', () => {
  ...
  it('keeps the failure on the dialog until the next edit clears it', async () => {
...
describe('a controller with no roster listener', () => {
  it('completes a delete without anyone to notify', async () => {
    ...
    const presets = seed()
    const alone = new AgentPresetSectionController(fakeRemote(presets, { id: 'standard' }))
    await alone.load()
...
describe('the model catalog', () => {
  ...
  it('loads the configured provider groups and failures', async () => {
    ...
    expect(calls.some(call => call.method === 'modelCatalog')).toBe(true)
  })
  ...
  it('keeps one catalog load in flight rather than stacking reads', async () => {
    ...
    expect(calls.filter(call => call.method === 'modelCatalog')).toHaveLength(1)
  })
