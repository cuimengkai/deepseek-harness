...
- **Backend-independent control.** The handler depends only on `compactNow(agent, signal)` and `compactRegionNow(start, end, agent, signal)`, so it works with any `CompactionEngine` implementation. The invoking agent is the exact target, and the dispatching UI's cancellation signal is forwarded through the seam.
...
- **Range argument grammar, undecided** — `<startSeq>:<endSeq>` is the current form; a later change would need a shared grammar across every command adapter.
