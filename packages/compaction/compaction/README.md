...
| `@deepseek-ai/dsh-command-compact` | Consumer: the human `/compact` command over `ctx.compaction.compactNow()` / `compactRegionNow()` |
...
| `compactRegionNow(start, end, agent, signal, sourceCommandId?)` | Explicitly compact one caller-selected surface range through the same idle-gated standalone transaction as `compactNow` — the range is the caller's choice instead of the retention policy's. `start` and `end` are surface seqs in positional order (the seq at the earlier position first), inclusive; both edges must be balanced. A missing, reversed, or unbalanced range rejects with a plain Error naming the violated edge; the other failures match `compactNow`. |
