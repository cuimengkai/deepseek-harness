...
`AgentOptions` 提供初始的提供方／模型路由，以及可选的正数 `maxTokens` 输出上限和可选的按类型 `modelKinds` 路由。具体循环会解析确切模型的适配器默认值，把生效上限记录到请求 header，并应用到每次对话模型请求；显式 Agent 选项优先，省略时由适配器或提供方路由默认值控制。`modelKinds` 把某一模型类型绑定到按类型的 provider/model 路由；它会携带进子 agent 的持久选项，但请求路由尚未消费类型。
