...
文档有六个区段，每个区段对应工作台的一个洞察标签：模块依赖拓扑、组件依赖、技术栈、组件、提示词与 agent 相关技术。每个输出的集合都按稳定键排序，每个路径都是相对项目根路径，因此扫描两次相同的受限树会得到逐字节一致的文档。文档携带两个身份——内容指纹，即排序的 `(relativePath, size, content)` 投影上的 sha256，仅在扫描时计算；stat 签名，即排序的 `(relativePath, size, mtimeMs)` 投影上的 sha256，读取路径把它与一次新鲜 stat 比较以判定新鲜度而不读取内容——而 `scannedAt` 是不参与两者的运行时元数据。磁盘上文档按类型存放在 `<root>/.dsh/insight/` 下：`meta.json` 承载带版本的身份字段，每个区段住在自己的类型化文件夹 `<section>/data.json` 中。agent 相关技术区段把项目内置的 agent 工具链作为受限的 markdown 集合嵌入——技能 `SKILL.md` 文件、MCP 服务器配置（每个 `env` 值都替换为 `<redacted>`，因为它们可能携带密钥）、以及 `.agents/prompts/`、`.claude/prompts/` 或根目录下的提示词文件——每条都在工作台的二级标签上以 markdown 渲染。
