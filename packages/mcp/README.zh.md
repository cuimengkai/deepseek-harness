...
`mcp/` 组把 harness 连接到 Model Context Protocol（MCP）工具服务器生态。`mcp-client` 挂载一台外部服务器——文件系统、GitHub、数据库或记忆服务器——让模型把它的工具当作原生工具、以稳定的服务器限定名称使用。`connector-registry` 把这些挂载持久化为通用 MCP 服务器卡片（URL 或 stdio），并为每张已启用卡片挂载 `mcp-client`。默认不启用任何服务器，因此按需逐台开启。只桥接 Tools 能力：MCP resources 与 prompts 不受支持。本页映射该组；逐包约定由包 README 负责。
...
| [`connector-registry/`](connector-registry/README.zh.md) | 持久化通用 MCP 服务器卡片，并为每张已启用卡片挂载 `mcp-client`（`ctx.connectors`） |
