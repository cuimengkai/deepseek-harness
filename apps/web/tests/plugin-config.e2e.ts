...
describe('web e2e: plugin configuration section', () => {
  ...
  /**
   * Open the settings page on the Plugins section. The scenarios share one
   * page so the settings document accumulates across them, so this leaves any
   * open settings page closed first — the covering page would otherwise
   * swallow the trigger click.
   */
  async function openPlugins() {
    if (await page.locator('[data-settings-page]').count() > 0) {
    ...
    await expect
      .poll(() => settings.getByRole('button', { name: '插件', exact: true }).getAttribute('aria-current'), { timeout: 5_000 })
      .toBe('page')
    await expect
      .poll(() => settings.getByRole('tab', { name: '插件配置', exact: true }).getAttribute('aria-selected'), { timeout: 5_000 })
      .toBe('true')
    return settings
  }
  ...
  it('shows one card per exposed host-plane namespace', async () => {
    ...
    const settings = await openPlugins()

    // Every card the shipped web composition exposes: the shell executor, the
    // agent loop, and the DeepSeek search provider.
    await settings.getByText('终端', { exact: true }).waitFor({ timeout: 10_000 })
    expect(await settings.getByText('Agent 循环', { exact: true }).count()).toBe(1)
    expect(await settings.getByText('网页搜索', { exact: true }).count()).toBe(1)
    // Collapsed: a card's fields appear only once it is expanded.
    expect(await settings.getByLabel('命令超时（毫秒）').count()).toBe(0)
  ...
  it('stages an edit and writes it only when saved', async () => {
    ...
    const settings = await openPlugins()
    await settings.getByText('终端', { exact: true }).click()

    const timeout = settings.getByLabel('命令超时（毫秒）')
    ...
    await expect.poll(() => settings.getByText('已覆盖').count(), { timeout: 5_000 }).toBe(1)
    expect(await settings.getByRole('button', { name: '恢复默认' }).count()).toBe(1)
  ...
  it('refuses to save a draft that is not a number', async () => {
    ...
    const save = settings.getByRole('button', { name: '保存', exact: true })
    await expect.poll(() => save.isDisabled(), { timeout: 5_000 }).toBe(true)
    expect(await settings.getByText('请填数字；留空表示使用默认值。').count()).toBe(1)
    await settings.getByRole('button', { name: '放弃修改' }).click()
    expect(tripwire.pageErrors).toEqual([])
  }, 60_000)
  ...
  it('clears the field back to the composed default on reset', async () => {
    ...
    await expect.poll(async () => (await settingsDocument()).includes('timeoutMs'), { timeout: 10_000 })
      .toBe(false)
    expect(await timeout.inputValue()).toBe('60000')
    expect(await settings.getByText('已覆盖').count()).toBe(0)
    expect(tripwire.pageErrors).toEqual([])
  }, 60_000)
