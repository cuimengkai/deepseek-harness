...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-router': { kind: 'none', reason: 'Browser-side URL history + page-route matching service; registers nothing model-facing.' },
  ...
  'packages/client/ui-context': { kind: 'indirect', reason: 'The tab submits the host /compact command through the commands Remote; the compaction engine owns the model-visible replacement, and contextComposition.read only reads the session log.' },
  ...
  'packages/host/plugin-manager': { kind: 'none', reason: 'Host-side home-patch install/uninstall management; registers nothing model-facing.' },
  ...
  'packages/session/context-composition': { kind: 'none', reason: 'The fold reads the durable log into one detached snapshot and writes nothing the model sees; no event, surface op, or request content is produced.' },
