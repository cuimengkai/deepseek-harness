...
2. **已结算的暂存清除 workspace pending。** workspaces 服务新增 `clearPendingAgentPreset()`，seat 通过新的 `onStageSettled` 回调报告暂存结算，该回调在暂存被应用、被已运行它的会话消费、或被拒绝时触发。每个点都清除 workspace pending，因此过期的暂存 preset 永不搭乘之后不相关的 connect。针对已开始会话做出的暂存会被保留而非结算——先于导入做出的 develop 选择必须到达下一个 connect（[develop-pick-survives-started-session](../bug-fix/2026-08-24-develop-pick-survives-started-session.zh.md)）。
...
- 已结算的暂存在所有地方被丢弃，因此之后的 connect 从部署默认开始；仍在等待已开始会话的暂存会存活到消费它的那个 connect。
