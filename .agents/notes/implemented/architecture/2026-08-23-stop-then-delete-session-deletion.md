...
4. **The client surfaces refusal.** `WorkspaceBrowser` renders a session-delete failure as an alert instead of a silent console note. The Delete action later gained a confirmation dialog, so the alert now lives inside that dialog rather than the list area ([[2026-08-26-session-delete-confirmation]]).
...
- A delete can still refuse when a scope member is live but not agent-owned (no factory mounted, or the factory reports false) — that refusal is now an alert in the UI, not a silent no-op. Since the confirmation dialog landed, the alert renders inside that dialog ([[2026-08-26-session-delete-confirmation]]).
