...
Split the on-disk layout into a meta file plus six typed section files under `.dsh/insight/`, keeping the in-memory document and the wire vocabulary unchanged.

```
.dsh/insight/
  meta.json                 # { formatVersion: 2, rootName, contentFingerprint, statSignature, scannedAt }
...
- `readDocument` reads the meta (absent → `undefined`), refuses a wrong-version or over-cap or unparsable meta, reads all six section files (any missing, over-cap, or unparsable → throw → read reports `error`), and reassembles the `ProjectInsightDoc`. The stat-only freshness comparison is unchanged ([[2026-08-24-read-freshness-stat-signature]]).
...
- The section-key → folder-name mapping is the only place the layout is declared: `techStack → tech-stack`, `moduleTopology → module-topology`, `componentDependencies → component-dependencies`, `components → components`, `prompts → prompts`, `agentTech → agent-tech`.

## Alternatives considered

- **Keep a single `project-insight.json` file** — rejected: it does not answer the per-type-folder request and keeps the six sections coupled under one byte guard.
...
- A scan commits seven files; a reader mid-write sees the previous document or `none`, never a partial one, because the meta file lands last.
