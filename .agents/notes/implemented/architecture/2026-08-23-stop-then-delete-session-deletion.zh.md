...
4. **客户端呈现拒绝。** `WorkspaceBrowser` 把会话删除失败渲染为 alert，取代静默的 console note。Delete 操作后来增加了确认对话框，该 alert 现在位于对话框内而非列表区（[[2026-08-26-session-delete-confirmation]]）。
...
- 当范围内成员活跃但非 agent 所有（未挂载工厂，或工厂返回 false）时删除仍可能拒绝——该拒绝现在是 UI 里的 alert，不再是静默无操作。确认对话框落地后，该 alert 渲染在对话框内（[[2026-08-26-session-delete-confirmation]]）。
