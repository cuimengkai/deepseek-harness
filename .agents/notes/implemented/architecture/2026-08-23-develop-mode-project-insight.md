...
`@deepseek-ai/dsh-project-insight` is a RELEASE host-plane package in the new `insight/` group: a deterministic offline scanner, a service with a session-lifecycle auto-scan hook, and the model-facing `scan_project` tool. The scanner is a pure function of the tree's bytes — no LLM, no network, no credentials — so scanning the same bounded tree twice yields a byte-identical document, and `scannedAt` is excluded from the content fingerprint. The document under `<root>/.dsh/insight/` carries six sections (module topology, component dependencies, tech stack, components, prompts, agent-related technology), every collection sorted by a stable key, every path root-relative, with hard caps (`MAX_SOURCE_FILES`, `MAX_EDGES`, `MAX_DOC_BYTES`) that bound both the wire and the browser render. The agent-related section embeds the project's built-in agent tooling as bounded content collections — skill `SKILL.md` files, MCP server configs (with every `env` value redacted to `<redacted>`, since they can carry secrets), and prompt files under `.agents/prompts/`, `.claude/prompts/`, or the root — each rendered as markdown on the workbench's second-level tabs; a fourth `documents` collection later embedded every other inventory file's content so the inventory sub-tab renders files, not metadata ([[2026-08-26-insight-agent-tech-documents]]).
...
The document reaches the browser through the privileged `projectInsight.read` RPC (reading project files is reconnaissance) and renders in `conversation.view` tabs owned by `@deepseek-ai/dsh-client-ui-project-insight` (order 20–60, after trajectory). Six tabs shipped initially; the prompts tab was later removed as a duplicate of the agent-tech tab's prompts sub-tab, leaving five ([[2026-08-26-insight-tabs-pinned-tree-focus]]). The conversation-view ring gains a per-session `modes` filter: an entry declaring `modes` shows only while the session's resolved preset is a member, so each mode owns its insight tabs and switching preset shows that mode's tabs by default. The shipped `develop` preset (`apps/cli/config/agent-presets/develop`) mounts `scan_project`, and its persona directs the model to scan on first workspace entry and after significant changes. The authoring bug is fixed by making the `mkdir` recursive, matching `replaceComposition`.
...
A develop-mode session over a project auto-scans once; other presets stay inert because the service gates on the resolved preset and a `cwd`. A preset switch into develop re-triggers a scan of a blank session's workspace. A second open of a scanned project reads the committed document instantly; an edit turns it `stale` and the next scan refreshes it. A committed document under an older format version self-heals instead of stranding in an error state: `read()` surfaces it as `stale` and schedules a debounced background rebuild, and `scan()` treats an unreadable stored document (wrong version, over-cap, missing section) as absent and rebuilds it — so bumping `PROJECT_INSIGHT_FORMAT_VERSION` (currently 4) rebuilds every existing project's committed doc on the next read. The document lives in the project's own tree, which the harness does not `.gitignore` — a project that commits everything will track the cache (recorded in the package README's Known Limitations). First scans of large trees are capped and best-effort (source scanning and heuristics, not a build or type-check). The `modes` filter makes the tab ring mode-aware generically: future modes declare their own tabs, and switching preset re-renders the ring without touching the trajectory plugin.
