 ...
 * with a session→platform-user binding. The service itself is mounted by the
 * composition row `id: platform-shell` (a file-backed SQLite control-plane
 * store); this plugin adds only the consumer (the market tools every agent
 * sees) and the actor binding the demo driver populates before each agent runs.
